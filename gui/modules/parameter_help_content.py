...
📦 解析缓存
• 说明：启用代码解析结果的两级缓存（内存+磁盘），大幅提升重复构建速度
...
• 适用场景：
  ...
  - ✅ 增量混淆配合使用
• 工作原理：
  - 首次混淆时解析所有文件，缓存到内存和磁盘
  - 后续混淆检查文件MD5，未变化的文件直接从缓存加载
  - 内存缓存命中：<1ms/文件（最快）
  - 磁盘缓存命中：~5ms/文件（较快）
  - 重新解析：~100ms/文件（慢）
• 推荐：**始终启用**（所有模板默认开启）
...
• **缓存目录**（cache_dir）:
  ...
  - 存储内容：
    ...
    * 代码解析结果（ParsedFile对象）
    * 缓存元数据和统计信息
  - 💡 提示：可添加到 `.gitignore` 避免提交

• **内存缓存大小**（max_memory_cache）:
  - 范围：1-10000
  - 默认值：1000
  - 说明：内存中最多缓存的文件数
  - 性能：
...
• **开发阶段**：启用缓存 + 增量混淆，最大化性能
• **CI/CD**：启用缓存，利用构建服务器的磁盘缓存
• **发布构建**：可选择clear_cache=True，确保使用最新代码
• **定期清理**：手动删除缓存目录，释放磁盘空间

💡 性能提升实例
- 100个文件首次混淆：10秒
- 修改5个文件后再次混淆：0.5秒（**20x加速**）
- 完全未修改再次混淆：0.1秒（**100x加速**）
