...
class MarsLogAnalyzerPro(OriginalMarsLogAnalyzerPro):
    ...
    def __init__(self, root):
        ...
        # AI交互管理器（先初始化为None，在create_widgets后再创建）
    ...
    def create_widgets(self):
        ...
        # 初始化AI交互管理器（在UI创建后）
        try:
            from modules.ai_interaction_manager import AIInteractionManager
            self.ai_manager = AIInteractionManager(self)
            # 设置AI功能
            self.ai_manager.setup_ai_features()
        except Exception as e:
            print(f"⚠️  AI功能初始化失败: {e}")
    ...
    # AI相关方法全部委托给ai_manager
    @property
    def ai_assistant(self):
        """AI助手属性（委托给ai_manager）"""
        if self.ai_manager:
            return self.ai_manager.ai_assistant
        return None

    def ai_analyze_selected_log(self, log_text=None):
        """委托给AI交互管理器"""
        if self.ai_manager:
            self.ai_manager.ai_analyze_selected_log(log_text)
        else:
            messagebox.showwarning("提示", "AI功能未初始化")

    def ai_explain_error(self, log_text=None):
        """委托给AI交互管理器"""
        if self.ai_manager:
            self.ai_manager.ai_explain_error(log_text)
        else:
            messagebox.showwarning("提示", "AI功能未初始化")
